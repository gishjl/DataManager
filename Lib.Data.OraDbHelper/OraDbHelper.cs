...
namespace Lib.Data.OraDbHelper
{
    /// <summary>
    ...
    public abstract class DbHelperOra
    {
        ...
        /// <returns></returns>
        public static OracleConnection GetConn(string connectionString)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                conn.Open();
        ...
        //写Blob字段到数据表
        public static int RwiteBlobToTable(string connectionString, string strTableName, string strBlobFieldName, string strFilter, string strBlob)
        {
            int iResult = -1;
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    // 获得 OracleLob 指针
                    cmd.CommandText = string.Format("select {0} from {1} where {2} FOR UPDATE", strBlobFieldName, strTableName, strFilter);
                    // 利用事务处理（必须）
                    ...
                    using (reader)
                    {
                        ...
                        tempLob.Erase();
                        byte[] btData = System.Text.Encoding.UTF8.GetBytes(strBlob);
                        tempLob.Write(btData, 0, btData.Length);
                    }
                    // 提交事务
                    transaction.Commit();
                }
        ...
        //
        public static int RwiteRegionBoundsBlobToTable(string connectionString, string strTableName, string strBlobFieldName, string strFilter, string strBlob)
        {
            int iResult = -1;
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    using (reader)
                    {
                        ...
                        OracleLob tempLob = reader.GetOracleLob(0);
                        tempLob.Erase();
                        byte[] boundsData = System.Text.Encoding.Default.GetBytes(strBlob);
                        tempLob.Write(boundsData, 0, boundsData.Length);
                    }
                    // 提交事务
                    transaction.Commit();
                }
